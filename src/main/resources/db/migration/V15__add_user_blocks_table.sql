...
CREATE TABLE user_blocks (
    ...
    CONSTRAINT ck_user_blocks_scope_validation CHECK (
        (scope_type = 'MOD' AND mod_id IS NOT NULL) OR
        (scope_type IN ('AUTHOR_GLOBAL', 'DIRECT_MESSAGES', 'INTERACTION') AND mod_id IS NULL)
    ),
...
CREATE UNIQUE INDEX uk_user_blocks_author_global_scope ON user_blocks (blocker_id, blocked_id) 
